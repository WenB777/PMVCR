...
def clustering(x_list, y):
    ...
    ret = {}
    ret['kmeans'] = scores
    return ret, y_preds.astype('int64')

def clustering_evalution(pred, y):
    """Get scores of clustering"""
    n_clusters = np.size(np.unique(y))

    if np.min(y) == 1:
        y = y - 1
    scores, _ = clustering_metric(y, pred, n_clusters)

    ret = {}
    ret['kmeans'] = scores
    return ret, pred.astype('int64')
...
def clustering_metric(y_true, y_pred, n_clusters, verbose=True, decimals=4):
    ...
    return dict({'ACC': accuracy, 'NMI': nmi, 'ARI': ari, 'F-score': f1_macro}), confusion_matrix
